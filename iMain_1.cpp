...
void addHighScore(string name, int score) {
    for (char &c : name) if (c == ' ') c = '_'; // Replace spaces with underscores for file storage
    highScores.push_back({name, score});
    sort(highScores.begin(), highScores.end());
...
void obstacle() {
    for (int i = 0; i < 3; i++) {
        if (isObsActive[i]) { // Only move active obstacles
            obsy[i] -= obstacleSpeed;
            if (obsy[i] < -obsimgfrm[obsi[i]].height) { // Check if obstacle is completely off-screen
        ...
        } else { // Obstacle is inactive (e.g., just collided)
            obsy[i] -= obstacleSpeed; // Still move it off-screen
            if (obsy[i] < -obsimgfrm[obsi[i]].height) { // Once off-screen, reactivate and reset
                isObsActive[i] = true;
                obsy[i] = 800;
...
void incmnt() { // Hot air balloon animation
    bi = (bi + 1) % 10; // Cycle through 10 frames
    // This line changes the frame for the current 'bi' sprite index.
    // Assuming habimg[bi] is the active sprite to be drawn.
    iChangeSpriteFrames(&habimg[bi], &habimgfrm[bi], 1);
    iSetSpritePosition(&habimg[bi], x, y);
}

void cldx() { // Cloud horizontal movement
...
void cldy() { // Cloud vertical movement
    double* cy[] = {&c1y, &c2y, &c3y, &c4y, &c5y, &c6y, &c7y};
    for (int i = 0; i < 7; i++) {
        *cy[i] -= 2; // Move downwards
        if (*cy[i] < -50) *cy[i] = 800; // Reset to top
    }
}
...
{
    if (gamestate == 3 && inputName) { // Only allow input on name screen
        if (key == '\b') { // Backspace
        ...
        } else if (playerNameIndex < (int)sizeof(playerName) - 1) { // Add character if space available
            if (isalnum(key) || key == ' ') { // Only allow alphanumeric characters and space
...
int main(int argc, char *argv[])
{
    ...
    // Get hot air balloon dimensions after loading its image
    if (10 > 0) {
