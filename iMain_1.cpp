...
int menuButtonX[] = {350, 850, 100, 610, 1120};
int menuButtonY[] = {260, 240, 60, 60, 60};
int menuButtonWidth = 200, menuButtonHeight = 200;
...
int balloonFrameIndex = 0;
int obstacleX[3], obstacleY[3], obstacleImageIndex[3];
int playerX = 700, playerY = 70;
int backgroundMusicHandle;
bool isObstacleActive[3] = {true, true, true};
bool showHeartbreakEffect = false;
...
char coinFileNames[14][15];
Image coinAnimationFrames[14];
...
const int BACK_BUTTON_X = 1300, BACK_BUTTON_Y = 700;

Image healthIconImage, coinIconImage;

const int UI_ICON_START_X = 10, UI_ICON_Y = 715, UI_ICON_GAP = 5;

int lastSpeedUpScore30 = 0, lastSpeedUpScore50 = 0;
...
HealthItem currentHealthItem;
Image healthItemImage;
int healthItemWidth, healthItemHeight;
const int MAX_PLAYER_HEALTH = 3;
int healthItemSpawnDelay = 0;
const int HEALTH_ITEM_SPAWN_INTERVAL = 3000;

// bird information
int currentBird = 0;
int birdX, birdY;
float birdSpeedX, birdSpeedY;
int bird1FrameIndex = 0;
int bird2FrameIndex = 0;
int bird3FrameIndex = 0;
...
void movePlayer()
{
    ...
    if (playerX + hotAirBalloonWidth > 1400)
        playerX = 0;
    if (playerX < 0)
        playerX = 1400 - hotAirBalloonWidth;
    if (playerY > 760)
        playerY = 760;
    if (playerY < 0)
        playerY = 0;
...
void manageObstacles()
{
    for (int i = 0; i < 3; i++)
    {
        ...
        if (obstacleY[i] < -obstacleFrames[obstacleImageIndex[i]].height)
        {
...
void moveCloudsX()
{
    for (int i = 0; i < 7; i++)
    {
        ...
        if (cloudSpeed[i] > 0 && cloudX[i] > 1450)
            cloudX[i] = -150;
        else if (cloudSpeed[i] < 0 && cloudX[i] < -150)
            cloudX[i] = 1450;
    }
}
...
void animateCoins()
{
    for (size_t i = 0; i < activeCoins.size(); ++i)
        if (activeCoins[i].active)
            activeCoins[i].frame = (activeCoins[i].frame + 1) % 14;
}
...
bool checkCoinObstacleCollision(int coinLeft, int coinRight, int coinBottom, int coinTop)
{
    for (int i = 0; i < 3; ++i)
    {
        if (isObstacleActive[i])
        {
            ...
            if (coinLeft < obstacleRight && coinRight > obstacleLeft && coinBottom < obstacleTop && coinTop > obstacleBottom)
                return true;
        }
    }
    return false;
...
void spawnCoin()
{
    ...
    if (coinIndex == -1 && activeCoins.size() < MAX_ACTIVE_COINS)
    {
        activeCoins.push_back({});
        coinIndex = activeCoins.size() - 1;
    }
    else if (coinIndex == -1)
        return;
    ...
    for (int retry = 0; retry < 50; ++retry)
    {
        ...
        for (size_t i = 0; i < activeCoins.size(); ++i)
        {
            if (activeCoins[i].active && i != (size_t)coinIndex)
            {
                ...
                if (distanceSq < pow(MIN_COIN_SEPARATION, 2))
                {
                    overlap = true;
                    break;
                }
...
void manageCoinMovement()
{
    ...
    for (size_t i = 0; i < activeCoins.size(); ++i)
    {
        if (activeCoins[i].active)
        {
            ...
            if (activeCoins[i].y < -coinHeight)
                activeCoins[i].active = false;
            activeCoinCount++;
        }
    }
...
// Checks for collision between a health item and other objects
bool checkHealthItemCollision(int itemLeft, int itemRight, int itemBottom, int itemTop)
{
    for (int i = 0; i < 3; ++i)
    {
        if (isObstacleActive[i])
        {
            int obstacleLeft = obstacleX[i], obstacleRight = obstacleX[i] + obstacleFrames[obstacleImageIndex[i]].width;
            int obstacleBottom = obstacleY[i], obstacleTop = obstacleY[i] + obstacleFrames[obstacleImageIndex[i]].height;
            if (itemLeft < obstacleRight && itemRight > obstacleLeft && itemBottom < obstacleTop && itemTop > obstacleBottom)
                return true;
        }
    }
    for (size_t i = 0; i < activeCoins.size(); ++i)
    {
        if (activeCoins[i].active)
        {
            ...
            if (itemLeft < coinRight && itemRight > coinLeft && itemBottom < coinTop && itemTop > coinBottom)
                return true;
        }
    }

    Sprite *activeBirdSprite = nullptr;
    Image *activeBirdImage = nullptr;
    if (currentBird == 1)
    {
        activeBirdSprite = &bird1Sprite;
        activeBirdImage = &bird1Frames[bird1FrameIndex];
    }
    else if (currentBird == 2)
    {
        activeBirdSprite = &bird2Sprite;
        activeBirdImage = &bird2Frames[bird2FrameIndex];
    }
    else if (currentBird == 3)
    {
        activeBirdSprite = &bird3Sprite;
        activeBirdImage = &bird3Frames[bird3FrameIndex];
    }
...
void spawnHealthItem()
{
    if (!currentHealthItem.active && playerHealth < MAX_PLAYER_HEALTH)
    {
        ...
        for (int retry = 0; retry < 50; ++retry)
        {
            ...
            {
                currentHealthItem.x = posX;
                currentHealthItem.y = posY;
...
// Resets and spawns a new bird
void spawnNewBird()
{
    currentBird = (rand() % 3) + 1;

    birdY = playerY + (rand() % 101 - 50);
    if (birdY > 725)
        birdY = 725;
    if (birdY < 0)
        birdY = 0;
...
void manageSpecialObstacles()
{
    ...
    {
        birdSpawnTimerTicks++;
        if (birdSpawnTimerTicks >= nextBirdSpawnTickTarget)
            spawnNewBird();
    }
    else
    {
        ...
        if (birdSpeedX > 0 && birdX > 1400)
        {
            isOffScreen = true;
        }
        else if (birdSpeedX < 0 && birdX < -currentBirdWidth)
        {
            isOffScreen = true;
        }
...
{
    iClear();
    iShowLoadedImage(0, 0, &skyImage);
    for (int i = 0; i < 7; i++)
        iShowLoadedImage(cloudX[i], cloudY[i], &cloudImage[i]);
    ...
    if (currentGameState == 0)
    {
        ...
        {
            if (isMenuHovered[i])
                iShowLoadedImage(menuButtonX[i] - 10, menuButtonY[i] - 10, &menuButtonBigImage[i]);
            else
                iShowLoadedImage(menuButtonX[i], menuButtonY[i], &menuButtonImage[i]);
        }
    }
    else if (currentGameState == 1)
    {
        iShowSprite(&hotAirBalloonSprites[balloonFrameIndex]);
        int playerLeft = playerX, playerRight = playerX + hotAirBalloonWidth, playerBottom = playerY, playerTop = playerY + hotAirBalloonHeight;
        ...
        for (int i = 0; i < 3; i++)
        {
            if (isObstacleActive[i])
            {
                ...
                if (iCheckCollision(&hotAirBalloonSprites[balloonFrameIndex], &obstacleSprites[i]))
                {
        ...
        for (size_t i = 0; i < activeCoins.size(); ++i)
        {
            if (activeCoins[i].active)
            {
                ...
                if (playerLeft < coinRight && playerRight > coinLeft && playerBottom < coinTop && playerTop > coinBottom)
                {
                    gameScore += 10;
                    coinsCollected++;
                    activeCoins[i].active = false;
        ...
        if (currentHealthItem.active)
        {
            ...
            if (playerLeft < healthItemRight && playerRight > healthItemLeft && playerBottom < healthItemTop && playerTop > healthItemBottom)
            {
        ...
        if (showHeartbreakEffect)
        {
            iShowLoadedImage((1400 - heartbreakImage.width) / 2, (800 - heartbreakImage.height) / 2, &heartbreakImage);
            if (--heartbreakCounter <= 0)
                showHeartbreakEffect = false;
        }

        char scoreText[30];
        sprintf(scoreText, "Score: %d", gameScore);
        iText(10, 770, scoreText, GLUT_BITMAP_HELVETICA_18);
        sprintf(scoreText, "Speed: %d", obstacleSpeed);
        iText(10, 750, scoreText, GLUT_BITMAP_HELVETICA_18);

        for (int i = 0; i < playerHealth; ++i)
            iShowLoadedImage(UI_ICON_START_X + (healthIconImage.width + UI_ICON_GAP) * i, UI_ICON_Y, &healthIconImage);
        iShowLoadedImage(10, 680, &coinIconImage);
        sprintf(scoreText, "%d", coinsCollected);
        iText(10 + coinIconImage.width + 5, 685, scoreText, GLUT_BITMAP_HELVETICA_18);
        iShowLoadedImage(BACK_BUTTON_X, BACK_BUTTON_Y, &backButtonImage);

        if (currentBird == 1)
            iShowSprite(&bird1Sprite);
        else if (currentBird == 2)
            iShowSprite(&bird2Sprite);
        else if (currentBird == 3)
            iShowSprite(&bird3Sprite);
    }
    else if (currentGameState == 3)
    {
        iSetColor(0, 0, 255);
        iText(560, 400, "Enter Your Name:", GLUT_BITMAP_TIMES_ROMAN_24);
        iText(680 - (playerNameIndex * 6), 370, playerName, GLUT_BITMAP_HELVETICA_18);
    ...
    {
        iSetColor(255, 0, 0);
        iText(625, 450, "GAME OVER", GLUT_BITMAP_TIMES_ROMAN_24);
        iSetColor(0, 0, 255);
        char scoreString[50];
        sprintf(scoreString, "FINAL SCORE: %d", finalScore);
        iText(575, 400, scoreString, GLUT_BITMAP_TIMES_ROMAN_24);
        sprintf(scoreString, "COINS COLLECTED: %d", coinsCollected);
        iText(550, 350, scoreString, GLUT_BITMAP_TIMES_ROMAN_24);
        iShowLoadedImage(BACK_BUTTON_X, BACK_BUTTON_Y, &backButtonImage);
    }
    else if (currentGameState == 2)
    {
        iSetColor(255, 255, 255);
        iText(625, 700, "High Scores", GLUT_BITMAP_HELVETICA_18);
        int startY = 650;
        for (size_t i = 0; i < highScores.size(); i++)
        {
            ...
            iText(700 - (strlen(scoreLine) * 5), startY - i * 30, scoreLine, GLUT_BITMAP_HELVETICA_18);
        }
        iShowLoadedImage(BACK_BUTTON_X, BACK_BUTTON_Y, &backButtonImage);
    }
}
...
{
    for (int i = 0; i < 5; i++)
        isMenuHovered[i] = (mouseX >= menuButtonX[i] && mouseX <= menuButtonX[i] + menuButtonWidth && mouseY >= menuButtonY[i] && mouseY <= menuButtonY[i] + menuButtonHeight);
}
...
void iMouse(int button, int state, int mouseX, int mouseY)
{
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        if (currentGameState == 0)
        {
            if (mouseX >= 350 && mouseX <= 350 + menuButtonWidth && mouseY >= 260 && mouseY <= 260 + menuButtonHeight)
            {
                currentGameState = 3;
            ...
            else if (mouseX >= 850 && mouseX <= 850 + menuButtonWidth && mouseY >= 240 && mouseY <= 240 + menuButtonHeight)
            {
                currentGameState = 2;
            }
            else if (mouseX >= 100 && mouseX <= 100 + menuButtonWidth && mouseY >= 60 && mouseY <= 60 + menuButtonHeight)
            {
                currentGameState = 5;
            }
            else if (mouseX >= 610 && mouseX <= 610 + menuButtonWidth && mouseY >= 60 && mouseY <= 60 + menuButtonHeight)
            {
                currentGameState = 4;
            }
            else if (mouseX >= 1120 && mouseX <= 1120 + menuButtonWidth && mouseY >= 60 && mouseY <= 60 + menuButtonHeight)
            {
                exit(0);
            }
        }
...
{
    if (currentGameState == 3 && isInputtingName)
    {
        if (key == '\b')
        {
        ...
        else if (key == '\r' || key == '\n')
        {
            ...
            currentGameState = 1;
            backgroundMusicHandle = iPlaySound("Easy.wav", true, 80);
            ...
            obstacleSpeed = 2;
            coinSpeed = obstacleSpeed;
            lastSpeedUpScore30 = 0;
            lastSpeedUpScore50 = 0;

            for (int i = 0; i < 3; i++)
            {
                obstacleY[i] = 800 + i * 267;
                obstacleX[i] = rand() % (1400 - obstacleFrames[obstacleImageIndex[i]].width);
                obstacleImageIndex[i] = rand() % 3;
                isObstacleActive[i] = true;
                iChangeSpriteFrames(&obstacleSprites[i], &obstacleFrames[obstacleImageIndex[i]], 1);
...
int main(int argc, char *argv[])
{
    ...
    coinWidth = coinAnimationFrames[0].width;
    coinHeight = coinAnimationFrames[0].height;

    // Load obstacle and balloon images
    for (int i = 0; i < 3; i++)
        iLoadImage(&obstacleFrames[i], obstacleFiles[i]);

    for (int j = 0; j < 10; j++)
        iLoadImage(&hotAirBalloonFrames[j], hotAirBalloonFiles[j]);
    hotAirBalloonWidth = hotAirBalloonFrames[0].width;
    hotAirBalloonHeight = hotAirBalloonFrames[0].height;
    ...
    // Load cloud images
    for (int i = 0; i < 7; i++)
        iLoadImage(&cloudImage[i], cloudFiles[i]);
    ...
    iLoadImage(&healthIconImage, "Healthicon.png");
    iResizeImage(&healthIconImage, 30, 30);
    iLoadImage(&coinIconImage, "Coinicon.png");
    iResizeImage(&coinIconImage, 30, 30);

    iLoadImage(&healthItemImage, "livesprite.png");
    iResizeImage(&healthItemImage, 30, 30);
    ...
    for (int i = 0; i < 9; i++)
    {
        iLoadImage(&bird1Frames[i], bird1files[i]);
        iResizeImage(&bird1Frames[i], 100, 100);
    }
    for (int i = 0; i < 6; i++)
    {
        iLoadImage(&bird2Frames[i], bird2files[i]);
        iResizeImage(&bird2Frames[i], 100, 100);
    }
    for (int i = 0; i < 10; i++)
    {
        iLoadImage(&bird3Frames[i], bird3files[i]);
        iResizeImage(&bird3Frames[i], 100, 100);
    }

    // Initialize bird sprites (off-screen)
    iInitSprite(&bird1Sprite);
    iInitSprite(&bird2Sprite);
    iInitSprite(&bird3Sprite);
